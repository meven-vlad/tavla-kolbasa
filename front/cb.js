  ...
  mkdirp(dirPath, function(err){                 // создаем
    ...
    // Нет ошибки, поехали!
      ...
      extensions.forEach(function(extention){
        ...
        let include; // есть ли инклуд блока
        let includeStrLook = blockName + '/' + blockName; // строа для проверки икклуда
        // Если это SCSS
        if(extention == 'scss') {
          ...
          fs.readFile('src/styles/style.scss', 'utf8', function(err, contents) { // читаем файл
            if(!err){
              include = contents.indexOf(includeStrLook);
              // подключаем если не было
              if (include < 0){
        ...
        // Если это PUG
        if(extention == 'pug') {
          templateFileImport = 'include ../blocks/' + blockName + '/' + blockName;
...
function removeBlock(){
  ...
  let includeStrScss = '\n'+"@import '../blocks/" + blockName + '/' + blockName + '\';'; // строка удаления из scss
  let includeStrPug = '\n'+'include ../blocks/' + blockName + '/' + blockName; // строка удаления из pug
